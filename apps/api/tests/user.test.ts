...
describe('GET /api/', () => {
  it('returns 403 when trying to list all users (admin-only feature)', async () => {
    ...
    expect(res.body.error).toMatch(/forbidden: user listing requires admin privileges/)
  })
  ...
  it('returns a user by id via ?id= (public profile for other users)', async () => {
    ...
    const res = await request(app).get('/api/?id=user_bob_002').set('Authorization', `Bearer ${TEST_TOKEN}`)
  ...
  it('returns 200 when looking up own email', async () => {
    ...
    const res = await request(app).get('/api/?email=alice@ouija.dev').set('Authorization', `Bearer ${TEST_TOKEN}`)
  ...
  it('searches users by query via ?q=', async () => {
    ...
    db.user.findMany.mockResolvedValueOnce(searchResults as any)

    const res = await request(app).get('/api/?q=alice').set('Authorization', `Bearer ${TEST_TOKEN}`)
...
describe('POST /api/', () => {
  it('creates a new user and returns 201', async () => {
    ...
    db.user.create.mockResolvedValueOnce(mockUser1 as any)
...
describe('PUT /api/:id', () => {
  ...
  it('returns 403 when trying to update another user', async () => {
    ...
    expect(res.status).toBe(403)
    expect(res.body.error).toMatch(/forbidden: you can only update your own profile/)
  })
...
describe('DELETE /api/:id', () => {
  it('deletes own user and returns 204', async () => {
    db.user.findUnique.mockResolvedValueOnce(mockUser1 as any)
    db.user.delete.mockResolvedValueOnce(mockUser1 as any)
  ...
  it('returns 403 when trying to delete another user', async () => {
    const res = await request(app).delete('/api/user_bob_002').set('Authorization', `Bearer ${TEST_TOKEN}`)

    expect(res.status).toBe(403)
    expect(res.body.error).toMatch(/forbidden: you can only delete your own account/)
  })
